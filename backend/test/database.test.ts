...
describe('Tests de base de données', () => {
    ...
    it('doit insérer un utilisateur', async () => {
        const result = await pool.query(
            ...
            ['tester1', 'tester1@example.com', 'hashed_password'],
        );
    ...
    it('doit insérer un second utilisateur', async () => {
        const result = await pool.query(
            'INSERT INTO users (name, email, password_hash) VALUES ($1, $2, $3) RETURNING *',
            ['tester2', 'tester2@example.com', 'hashed_password'],
        );
    ...
    it('ne doit pas insérer un utilisateur avec le même mail', async () => {
        try {
            await pool.query(
                'INSERT INTO users (name, email, password_hash) VALUES ($1, $2, $3) RETURNING *',
                ['tester3', 'tester1@example.com', 'hashed_password'],
            );
    ...
    it('doit insérer une colonne', async () => {
        ...
        await pool.query(
            'INSERT INTO columns (board_id, name) VALUES ($1, $2)',
            [boardId, 'En cours'],
        );
    ...
    it('doit insérer une tâche', async () => {
        ...
        await pool.query(
            'INSERT INTO tasks (column_id, name, description) VALUES ($1, $2, $3)',
            [columnId, 'tâche test', 'Ceci est une tâche test.'],
        );
