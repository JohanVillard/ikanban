...
describe('Tests de base de données', () => {
	...
	it('doit insérer un utilisateur', async () => {
		const result = await pool.query(
			...
			['75a745c9-975f-4826-a263-13d1dbb2eec5', 'tester1', 'tester1@example.com', 'hashed_password'],
		);
	...
	it('doit insérer un second utilisateur', async () => {
		const result = await pool.query(
			'INSERT INTO users (id, name, email, password_hash) VALUES ($1, $2, $3, $4) RETURNING *',
			['2691cd19-1633-46cc-a7b6-15f98e457e71', 'tester2', 'tester2@example.com', 'hashed_password'],
		);
	...
	it('ne doit pas insérer un utilisateur avec le même mail', async () => {
		try {
			await pool.query(
				'INSERT INTO users (id, name, email, password_hash) VALUES ($1, $2, $3, $4) RETURNING *',
				['780afe11-6d37-4cd3-9d96-b444af4ff06e', 'tester3', 'tester1@example.com', 'hashed_password'],
			);
	...
	it('ne doit pas insérer une colonne sans board_id valide', async () => {
		try {
			...
			await pool.query(
				'INSERT INTO columns (id, board_id, name) VALUES ($1, $2, $3)',
				["de87cd7c-dc89-4f21-8ac4-2e2d2ac49612", invalidUUID, 'En attente'],
			);
	...
	it('doit insérer une tâche', async () => {
		...
		await pool.query(
			...
			["d149a698-ad09-4518-a671-336526e97918", columnId, 'tâche test', 'Ceci est une tâche test.'],
		);
